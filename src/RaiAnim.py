...
colorDataBase = {
    "purplePosy ":  (236/256.0, 223/256.0, 237/256.0, 1.0), "rococoRose ":  (221/256.0, 146/256.0, 155/256.0, 1.0), "seasideSpray ":  (183/256.0, 204/256.0, 226/256.0, 1.0), "blushingBride ":  (249/256.0, 194/256.0, 188/256.0, 1.0)  # cherry red
    ...
    # magenta-ish
    , "highlandHeather ":  (173/256.0, 153/256.0, 200/256.0, 1.0)
}
    ...
    def __init__(self, fname):
        ...
        while True:
            ...
            if p is not None:
                ...
                timeStart = p.group(2)
                print("Segment ", ctrSegments,
                      " starts at timeframe ", timeStart)
                ...
                self.plan_path = []
                for k in range(0, numPlanSteps):
                    pos_k = np.fromstring(f.readline(), dtype='float', sep=' ')
                    self.plan_path.append(pos_k)

        f.close()

        print(self.numSegments+" animation segments found.")
        print(f"number of plan steps: {len(self.plan_path)}")
